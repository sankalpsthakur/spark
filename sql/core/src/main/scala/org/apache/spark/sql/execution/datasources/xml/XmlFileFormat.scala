...
case class XmlFileFormat() extends TextBasedFileFormat with DataSourceRegister {
  ...
  override def buildReader(
    ...
    val actualRequiredSchema =
      ExprUtils.dropCorruptRecordField(requiredSchema, columnNameOfCorruptRecord)
    if (requiredSchema.length == 1 &&
      ExprUtils.isCorruptRecordColumn(requiredSchema.head.name, columnNameOfCorruptRecord)) {
